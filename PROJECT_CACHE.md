...
- **Color Link** (`puzzles_colorlink.json`): array of `{date?, rows, cols, flows:[{color, a:[r,c], b:[r,c], path:[[r,c]...]}]}` (non-square 12×9; legacy square `size` still accepted by `ph_colorlink_make`). `a`/`b` = endpoint dots; `path` = full solution route (powers the hint + win recap). Generated via Hamiltonian-path-cut over ROWS×COLS so flows always tile the board, 9–10 flows/board, 4–15 cells each (`tools/gen_colorlink.py`; shorter/more flows = easier solve). Two-pass date select; hardcoded 12×9 fallback.
